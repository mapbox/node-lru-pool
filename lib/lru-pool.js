...
LRUPool.prototype.expire = function (key, callback) {
  if (callback && !(callback instanceof Function)) {
    throw new Error('Callback must be a function')
  }

  var lists = ['available', 'busy']
  for (var i = 0; i < lists.length; i++) {
    for (var walker = priv(this, lists[i]).head; walker !== null;) {
      var next = walker.next
      var hit = walker.value

      // Flag matching object as expired
      if (hit.key === key) hit.maxAge = -1
