  ...
  if (priv(self, 'cache').has(key)) {
    ...
    if (isStale(self, hit) && !priv(self, 'allowStale')) {
      ...
      if (priv(self, 'destroy') && priv(self, 'destroyStale')) {
        priv(self, 'destroy').call(self, hit)
      }
    ...
    if (isStale(this, node.value)) {
      if (priv(this, 'destroyStale')) {
        ...
        if (priv(this, 'destroy')) {
          priv(this, 'destroy').call(this, hit)
        }
      } else {
  ...
  if (length > priv(self, 'max')) {
    for (var walker = priv(self, 'available').tail; length > priv(self, 'max') && walker !== null;) {
      ...
      if (priv(self, 'destroy')) {
        priv(self, 'destroy').call(this, hit.value)
      }
...
function recycle (self, key, callback) {
  ...
  if (!node || length < priv(self, 'max')) {
    hit = create(self, key, now, maxAge)
  ...
  if (priv(self, 'init')) {
    priv(self, 'init').call(this, key, hit.value, function (err, obj) {
      hit.value = obj
      callback(err, key, hit.value)
    })
  } else {
    callback(null, key, hit.value)
  }
...
function create (self, key, now, maxAge) {
  return new Entry(self, key, now, maxAge)
}

// Classy, since V8 prefers predictable objects
function Entry (self, key, now, maxAge) {
  this.key = key
  this.value = priv(self, 'create').call(this)
  this.now = now
