...
LRUPool.prototype.acquireOnce = function (key, callback) {
  if (!(callback instanceof Function)) {
    throw new Error('Callback must be a function')
  }

  recycle(this, key, -1, callback)
}
...
function acquire (self, key, callback) {
  ...
  if (priv(self, 'cache').has(key)) {
    ...
    recycle(self, key, maxAge, callback)
  }
}
...
function recycle (self, key, maxAge, callback) {
  ...
  if (!node || length < priv(self, 'max')) {
    hit = create(self, key, now, maxAge, function (err, hit) {
...
function create (self, key, now, maxAge, callback) {
  var hit = new Entry(self, key, now, maxAge)
